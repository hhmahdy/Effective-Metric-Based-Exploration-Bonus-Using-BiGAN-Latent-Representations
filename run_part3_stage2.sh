#!/usr/bin/env bash
#
# Part-3 Stage-2 launcher on ALE/MontezumaRevenge-v5.
#
# Stage 2 runs the controlled representation-intervention experiment
# (BiGAN + N_T / IDF + N_T / RND + N_T) on the Atari game
# ALE/MontezumaRevenge-v5 (gymnasium/ALE v5 defaults, no extra wrappers).
#
...
echo "[stage2] repository root: $(pwd)"
echo "[stage2] environment id:  ${ENVIRONMENT_ID}"
...
echo "[stage2] preflight: '${PYTHON_BIN}' import main -> OK"
...
echo "[stage2] preflight: budget divisibility -> OK (${TOTAL_ENVIRONMENT_STEPS} / ${SAMPLES_PER_UPDATE} = $(( TOTAL_ENVIRONMENT_STEPS / SAMPLES_PER_UPDATE )) updates)"

# 3. Environment registration (ALE). Print the ALE env ID and proceed if it
#    resolves via gymnasium.spec; on failure exit with the resolution error.
echo "[stage2] checking ALE environment '${ENVIRONMENT_ID}' registration..."
if ! "${PYTHON_BIN}" -c "import gymnasium, ale_py, gymnasium as g; g.spec('${ENVIRONMENT_ID}')" >/dev/null 2>&1; then
  echo "ERROR: ALE environment '${ENVIRONMENT_ID}' could not be resolved via gymnasium.spec." >&2
  echo "       Ensure the ALE/gymnasium[atari] package and the exact environment id are available, then re-run." >&2
  exit 1
fi
echo "[stage2] preflight: '${ENVIRONMENT_ID}' spec registration -> OK"

# 4. Output directory tree.
mkdir -p "${OUTPUT_ROOT}"/{raw,processed,figures,checkpoints,logs} \
         "${OUTPUT_ROOT}/logs/stage${STAGE_NUM}"
echo "[stage2] preflight: output tree under ${OUTPUT_ROOT} -> OK"
...
for seed in ${SEEDS}; do
  for rep in ${REPRESENTATIONS}; do
    ...
    if [ -f "${run_dir}/metrics.jsonl" ] && [ "${FORCE_RERUN}" != "1" ]; then
      echo "[stage2] skip seed=${seed} rep=${rep}: run dir already has metrics.jsonl (set FORCE_RERUN=1 to rerun) -> ${run_dir}"
    ...
    if [ "${FORCE_RERUN}" = "1" ] && [ -d "${run_dir}" ]; then
      ...
      echo "[stage2] FORCE_RERUN=1: moving existing run dir to ${backup}"
      mv "${run_dir}" "${backup}"
    fi

    mkdir -p "${run_dir}"
    echo "[stage2] launching seed=${seed} rep=${rep} -> ${run_dir}"
    ...
    if [ "${exit_code}" -ne 0 ]; then
      echo "[stage2] FAILED seed=${seed} rep=${rep} exit_code=${exit_code} run_dir=${run_dir}"
      FAILED=1
      STATUS_ENTRIES="${STATUS_ENTRIES}${key}:FAILED:${run_dir}\n"
    else
      echo "[stage2] OK seed=${seed} rep=${rep} exit_code=0 run_dir=${run_dir}"
...
echo ""
echo "===================================================================="
echo "Stage 2 summary (seed/rep -> status)"
echo "===================================================================="
...
if [ "${FAILED}" -ne 0 ]; then
  echo ""
  echo "ERROR: one or more Stage 2 arms failed."
  exit 1
fi

echo ""
echo "[stage2] all arms completed (see manifest above)."
